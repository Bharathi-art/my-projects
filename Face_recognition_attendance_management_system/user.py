...
def admin_login():
    ...
    try:
        ...
        query = "SELECT * FROM student_registration  WHERE Student_name = %s"
        cursor.execute(query, (username,))
        result = cursor.fetchone()

        if result:
            if result[1] == password:
                messagebox.showinfo("Student Login", "Login Successful")
                # Execute the Python script upon successful login
                os.system('Python user_panel.py')
                root.destroy()  # Close the login window
            else:
                messagebox.showerror("Student Login", "Incorrect Password")
        else:
            messagebox.showerror("Student Login", "Username not found")

        cursor.close()
        conn.close()

    except mysql.connector.Error as err:
        messagebox.showerror("Student Login", f"Database Error: {err}")
